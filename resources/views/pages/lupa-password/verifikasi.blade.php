<x-layouts.app>
    <x-slot:title>
        Verifikasi Reset Password — JogjaTouch
    </x-slot:title>
    ...
    <main id="otp-bg" class="min-h-[80vh] flex items-center justify-center py-16 relative overflow-hidden" style="background: #ffffff;">
        ...
        <div class="relative w-full max-w-md px-6" style="z-index:10;">
            ...
            <div class="relative bg-white rounded-[2.5rem] p-8 md:p-10 shadow-xl border border-[#1E1B19]/5">
                ...
                <!-- Close Button -->
                <button onclick="window.location.href='{{ route('lupa-password') }}'" class="absolute top-6 right-6 w-9 h-9 rounded-full bg-[#1E1B19]/5 flex items-center justify-center text-[#1E1B19]/70 hover:bg-[#1E1B19]/10 transition-colors" aria-label="Close">
                ...
                <div id="step-otp-verification">
                    <div class="text-center mb-6">
                        <h3 class="font-serif-display text-3xl font-extrabold tracking-tight text-[#1E1B19]">
                            Verifikasi <span class="text-[#E35D25] italic font-semibold">Reset</span>
                        </h3>
                    ...
                    <!-- Back Button -->
                    <a href="{{ route('lupa-password') }}" class="inline-flex items-center gap-1.5 text-xs font-semibold text-[#1E1B19]/60 hover:text-[#E35D25] mb-4 transition-colors">
                        &larr; Ganti email / mulai ulang
                    </a>
                    ...
                    <!-- Verification Form -->
                    <form method="POST" action="{{ route('lupa-password.verify-otp') }}">
                        @csrf
                        <!-- 6 Digit Code Input -->
                        <div class="mb-6">
